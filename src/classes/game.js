...
export class Game {
    ...
    playComputerTurn(humanPlayer) {
        const getSmallestShipSize = function(ships) {
            const unsunkShips = ships.filter(ship => !ship.isSunk());
            return unsunkShips.reduce((acc, cur) => acc.size < cur.size ? acc : cur).size;
        }

        const isVertical = function(startRow, endRow) {
            return startRow !== endRow;
        }

        const playEasyTurn = function(board, ships, available, hits) {
            ...
            const smallestShip = getSmallestShipSize(ships);
            return slidingWindowTraversal(smallestShip);
        }

        const playMediumTurn = function(board, ships, available, hits) {
        ...
        const playHardTurn = function(board, available, hits) {
            const TRAVERSE_DIRECTIONS = [-1,1];
            ...
            const traverseFromShipLocation = function(queue, visited) {
                const CAP = 4;
                let count = 0;
            ...
            const findTargetsNearShips = function() {
                board.forEach((row, rowIndex) => {
                    row.forEach((col, colIndex) => {
                        if (col.ship !== null && col.hit === false) {
                            ...
                            traverseFromShipLocation(queue, visited);
                        };
                    })
                })
                return targets[Math.floor(Math.random() * targets.length)]; 
        ...
        if (this.difficulty === 0) target = playEasyTurn(gameBoard, ships, available, hits);
        else if (this.difficulty === 1) target = playMediumTurn(gameBoard, ships, available, hits);
        else target = playHardTurn(gameBoard, available, hits);
